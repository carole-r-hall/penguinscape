...
empirical_density = 1.66736 # empirical density for BEAG; modify this depending on colony's average density

RDF_savefile = '../output/output_csvs/beagle_least_squares.csv'
curve_file = '../output/output_csvs/beagle_curves.csv'
RDF_file = '../data/beagle_RDF.csv'

# create csv for least squares RDF minimization
with open(RDF_savefile, 'w', newline='') as csvfile:
    fieldnames = ['sigma', 'epsilon', 'LSD_RDF']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

# create CSV to store curves
with open(curve_file, 'w', newline = '') as csv_2:
    fieldnames2 = ['sigma', 'epsilon', 'curve', 'bins']
    writer = csv.DictWriter(csv_2, fieldnames = fieldnames2)
...
with open(RDF_file, newline='') as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
        empirical_rdf.append(float(row['gofr']))
        empirical_dists.append(float(row['bin_centers']))
...
class simulate_nests:
    ...
    def timestep(self, deltat, volume):
        self.nsteps += 1
...
epsilons = np.linspace(0.05,1.0,30)
sigmas = np.linspace(0.5, 1.0, 30) # when you find a value that works, make the grid finer around there to optimize


'''-----------------GRID SEARCH SIMULATION-----------------'''

for epsilon, sigma in list(itertools.product(epsilons, sigmas)):
    print("-------------BEGIN SIM----------------")
    ...
    curves = [[] for i in range(len(epsilons)*len(sigmas))]
    
    for i in range(1,NSteps):
        sim.timestep(dt, volume)
    ...
    data = [str(sigma), str(epsilon), str(sum_sq)]
    with open(r'../output/output_csvs/beagle_least_squares.csv', 'a') as f:
        writer = csv.DictWriter(f , fieldnames = fieldnames)
        writer.writerow({'sigma': sigma, 'epsilon': epsilon, 'LSD_RDF': sum_sq})

    # append curve to csv file
    data2 = [str(sigma), str(epsilon), str(avg_rdf), str(dists)]
    with open(r'../output/output_csvs/beagle_curves.csv', 'a') as f2:
        writer2 = csv.DictWriter(f2, fieldnames = fieldnames2)
        writer2.writerow({'sigma': sigma, 'epsilon': epsilon, 'curve': avg_rdf, 'bins': dists})
    ...
    plt.plot(dists, avg_rdf, c = 'palevioletred', linewidth = 2.0)
    plt.plot(empirical_dists, empirical_rdf, c = 'darkslateblue', linewidth = 2.0)
    plt.title("Empirical vs. Experimental RDF for Beagle Island at sigma = {s}, epsilon = {e}".format(s = sigma, e = epsilon))
    plt.savefig("../output/output_imgs/rdf_s_{s}_e_{e}.png".format(s = sigma, e = epsilon), dpi=300)
    ...
    plt.savefig("../output/output_imgs/energetics_s_{s}_e_{e}.png".format(s = sigma, e = epsilon), dpi=300)

    print("done with current step")
